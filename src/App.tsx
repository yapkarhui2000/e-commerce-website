...
function App() {
  ...
  // ===== Hooks =====
  useEffect(() => updateSearchedProducts(), [searchTerm,sortingOpt,quantityCheck]);
  ...
  function updateSearchedProducts(){
    ...
    if(searchTerm !== ''){
      holderList = holderList.filter((product: Product)=>product.name.toLowerCase().includes(searchTerm.toLowerCase ()));
    }
    if(quantityCheck){
  ...
  return (
    <div id="container"> 
                ...
                {addBasket.map(item=>(
                  <div className='shopping-row' key = {item.product.id}>
                    <div className='shopping-information'>
                      ...
                      <button onClick={()=>itemRemoval(item.product)}>Remove</button>
                      </div>
                ))}
                <p>
                  Total: £
                  {addBasket.reduce((total,item)=> total+ item.product.price *item.quantity,0).toFixed(2)}
                </p>
                </div>
            )}
            </div>
      <div id="search-bar">
        ...
        <div id="control-area">
          <select value={sortingOpt} onChange={a=>{setSortingOpt(a.target.value)}}>
          ...
          <input id="inStock" type="checkbox" checked={quantityCheck} onChange={a=>setQuantityCheck(a.target.checked)}></input>
